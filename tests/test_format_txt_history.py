...
@pytest.mark.asyncio
async def test_main_combines_files(tmp_path, monkeypatch):
    """Test that main function combines multiple message files when found."""
    from format_txt_history_full import main  # Move import to top of file

    # Create test files
    phone_file = tmp_path / "+18673335566.txt"
    email_file = tmp_path / "apple@phil-g.com.txt"

    # Write test content in a more readable way
    phone_content = "Dec 17, 2024  4:31:51 PM\nJess\nMessage 1\n\n"
    email_content = "Dec 17, 2024  5:31:51 PM\napple@phil-g.com\nMessage 2\n\n"

    phone_file.write_text(phone_content)
    email_file.write_text(email_content)

    # Run main with both files
    await main()  # Changed to await since main is async
    ...
    content = "".join(f.read_text() for f in chunk_files)
...
def test_parse_messages():
    # Break up test data into multiple lines
    test_message = (
