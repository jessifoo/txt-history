...
impl InputValidator {
    ...
    pub fn validate_phone(phone: &str) -> Result<()> {
        ...
        if !cleaned.starts_with('+') && !cleaned.chars().all(|c| c.is_ascii_digit()) {
            return Err(anyhow!("Phone number must start with + or contain only digits"));
        }
    ...
    pub fn validate_file_path(path: &Path) -> Result<()> {
        ...
        if path_str.contains("..") || path_str.contains("~") {
            return Err(anyhow!("File path contains potentially dangerous characters"));
        }
    ...
    /// Validate date range
    pub fn validate_date_range(start: Option<DateTime<Local>>, end: Option<DateTime<Local>>) -> Result<()> {
    ...
    pub fn validate_processing_version(version: &str) -> Result<()> {
        ...
        if !version.chars().all(|c| c.is_ascii_alphanumeric() || c == '.' || c == '-' || c == '_') {
    ...
    pub fn validate_imessage_db_path(path: &Path) -> Result<()> {
        ...
        if std::fs::metadata(path).map_err(|e| anyhow!("Cannot access iMessage database: {}", e))?.permissions().readonly() {
